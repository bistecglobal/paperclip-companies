...
# Part of the specpaper skill. Bash + coreutils only (jq optional).
set -euo pipefail
...
usage() {
  cat <<'EOF'
Usage: verify-context.sh <specpaper_dir> <change_name>
...
# Validate
[ -d "$SPECPAPER_DIR" ] || die "specpaper directory not found: ${SPECPAPER_DIR}"
...
EVIDENCE_JSON=$("$SCRIPT_DIR/verify.sh" collect "$SPECPAPER_DIR" "$CHANGE_NAME")
