...
usage() {
  cat <<'EOF'
Usage:
  log-error.sh <specpaper_dir> <change_name> <task_id> <wave> <agent_label> "<summary>" [--error-file <path>]
  log-error.sh <specpaper_dir> <change_name> --resolve <task_id>

Log mode:
  Appends a structured error entry to <specpaper_dir>/changes/<change_name>/errors.md.
...
SPECPAPER_DIR="$1"
CHANGE_NAME="$2"
ERRORS_MD="${SPECPAPER_DIR}/changes/${CHANGE_NAME}/errors.md"
