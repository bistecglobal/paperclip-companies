...
usage() {
  cat <<'EOF'
Usage:
  log-learning.sh <specpaper_dir> <change_name> <category> <priority> "<detail>" ["<action>"]
  log-learning.sh <specpaper_dir> <change_name> --list
  log-learning.sh <specpaper_dir> <change_name> --promote <learning_id>
...
LEARNINGS_FILE="$SPECPAPER_DIR/learnings.md"
...
# --- Log mode ---
if [[ $# -lt 5 ]]; then
  echo "Error: Log mode requires: <specpaper_dir> <change_name> <category> <priority> \"<detail>\" [\"<action>\"]" >&2
  exit 1
fi
