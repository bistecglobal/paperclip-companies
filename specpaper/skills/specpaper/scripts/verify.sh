#!/usr/bin/env bash
# verify.sh — Evidence collection and test execution for specpaper verification
# Part of the specpaper skill. Bash + coreutils only (jq optional for validation).
set -euo pipefail
...
cmd_collect() {
  local specpaper_dir="$1"
  local change_name="$2"
  local change_dir="${specpaper_dir}/changes/${change_name}"

  # Validate required files
  local spec_file="${change_dir}/spec.md"
  local tasks_file="${change_dir}/tasks.md"
  local config_file="${specpaper_dir}/config.yaml"
  ...
  for fp in "${unique_paths[@]}"; do
    local full_path
    # Resolve relative to project root (specpaper_dir parent or repo root)
    if [[ "$fp" = /* ]]; then
      full_path="$fp"
    else
      # Try relative to specpaper_dir first, then cwd
      if [ -f "${specpaper_dir}/${fp}" ]; then
        full_path="${specpaper_dir}/${fp}"
      elif [ -f "$fp" ]; then
        full_path="$fp"
      else
        full_path="${specpaper_dir}/${fp}"
...
case "${1:-}" in
  -h|--help)
    ...
    ;;
  collect)
    [ $# -ge 3 ] || die "Usage: verify.sh collect <specpaper_dir> <change_name>"
    cmd_collect "$2" "$3"
    ;;
  report)
    [ $# -ge 3 ] || die "Usage: verify.sh report <specpaper_dir> <change_name>"
    cmd_report "$2" "$3"
    ;;
  update-status)
    [ $# -ge 4 ] || die "Usage: verify.sh update-status <specpaper_dir> <change_name> <verdict>"
