...
# Usage: update-status.sh <specpaper_dir>

set -euo pipefail

SPECPAPER_DIR="$1"
CHANGES_DIR="$SPECPAPER_DIR/changes"
ARCHIVE_DIR="$CHANGES_DIR/archive"
STATUS_FILE="$SPECPAPER_DIR/STATUS.md"

# Read project name from config
PROJECT_NAME=$(grep 'name:' "$SPECPAPER_DIR/config.yaml" | head -1 | sed 's/.*name: *"\(.*\)"/\1/')
