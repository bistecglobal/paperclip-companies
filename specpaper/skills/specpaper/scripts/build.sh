#!/usr/bin/env bash
# build.sh — Build engine for specpaper: setup, commit, finalize
# Part of the specpaper skill. Bash + git + coreutils only.
set -euo pipefail
...
Subcommands:
  setup         <specpaper_dir> <change_name>
                Read config.yaml, create git branch/worktree if needed, output config JSON.

  commit        <specpaper_dir> <change_name> <task_id> "<title>" [files...]
                Stage listed files and commit with specpaper prefix.

  finalize      <specpaper_dir> <change_name>
                Run configured checks, merge branch if branch/worktree-per-change, output summary JSON.

  worktree-path <specpaper_dir> <change_name>
...
cmd_setup() {
  [[ $# -ge 2 ]] || die "setup requires: <specpaper_dir> <change_name>"
  local specpaper_dir="$1" change_name="$2"
  local config="$specpaper_dir/config.yaml"
  ...
  branch_prefix="${branch_prefix:-specpaper/}"
  auto_commit="${auto_commit:-true}"
  commit_prefix="${commit_prefix:-specpaper}"
  ...
  if [[ "$git_strategy" == "worktree-per-change" ]]; then
    local worktree_dir="${worktree_dir_cfg:-.specpaper/worktrees}"
...
cmd_commit() {
  [[ $# -ge 4 ]] || die "commit requires: <specpaper_dir> <change_name> <task_id> <title> [files...]"
  local specpaper_dir="$1" change_name="$2" task_id="$3" title="$4"
  shift 4
  local files=("$@")
  ...
  if [[ "$git_strategy" == "worktree-per-change" ]]; then
    ...
    worktree_dir="${worktree_dir:-.specpaper/worktrees}"
...
cmd_finalize() {
  [[ $# -ge 2 ]] || die "finalize requires: <specpaper_dir> <change_name>"
  local specpaper_dir="$1" change_name="$2"
  local config="$specpaper_dir/config.yaml"
  [[ -f "$config" ]] || die "Config not found: $config"
...
cmd_worktree_path() {
  [[ $# -ge 2 ]] || die "worktree-path requires: <specpaper_dir> <change_name>"
  local specpaper_dir="$1" change_name="$2"
  local config="$specpaper_dir/config.yaml"
  [[ -f "$config" ]] || die "Config not found: $config"
  local worktree_dir="$(yaml_val "$config" "git.worktree_dir")"
