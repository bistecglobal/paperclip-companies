...
usage() {
  cat <<'EOF'
Usage:
  detect-patterns.sh <specpaper_dir> scan <change_name>
  detect-patterns.sh <specpaper_dir> list [--min-recurrence N]
  detect-patterns.sh <specpaper_dir> promote <pat-id>
...
ensure_patterns_file() {
  ...
  local patterns_file="$specpaper_dir/.specpaper/patterns.md"
...
do_scan() {
  local specpaper_dir="$1"
  local change_name="$2"
  local changes_dir="$specpaper_dir/.specpaper/changes/$change_name"
  local patterns_file
  patterns_file=$(ensure_patterns_file "$specpaper_dir")
  local today
  today=$(date +%Y-%m-%d)
...
case "$MODE" in
  scan)
    ...
    do_scan "$SPECPAPER_DIR" "$1"
    ;;
  list)
    do_list "$SPECPAPER_DIR" "$@"
    ;;
  promote)
    if [[ $# -lt 1 ]]; then
