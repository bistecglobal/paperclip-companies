...
# Usage: validate-change.sh <specpaper_dir> <change_name> <phase|status>
#
...
Arguments:
  specpaper_dir   Path to the .specpaper directory
...
Examples:
  validate-change.sh .specpaper my-feature build
  validate-change.sh .specpaper my-feature status
EOF
  exit 0
}
...
# --- Args ---
if [[ $# -lt 3 ]]; then
  echo "ERROR: Expected 3 arguments: <specpaper_dir> <change_name> <phase|status>" >&2
...
if [[ ! -d "$SPECPAPER_DIR" ]]; then
  echo "ERROR: specpaper directory not found: $SPECPAPER_DIR" >&2
  exit 1
fi

CHANGE_DIR="$SPECPAPER_DIR/changes/$CHANGE_NAME"
CONFIG_FILE="$SPECPAPER_DIR/config.yaml"
...
case "$PHASE" in
  ...
  plan)
    if [[ ! -f "$CHANGE_DIR/proposal.md" ]]; then
      fail "Missing proposal.md — run specpaper propose first"
    fi
  ...
  build)
    if [[ ! -f "$CHANGE_DIR/spec.md" ]]; then
      fail "Missing spec.md — run specpaper plan first"
    fi
    if [[ ! -f "$CHANGE_DIR/design.md" ]]; then
      fail "Missing design.md — run specpaper plan first"
    fi
    if [[ ! -f "$CHANGE_DIR/tasks.md" ]]; then
      fail "Missing tasks.md — run specpaper plan first"
    fi
    # Enforce GitHub issue if sync is configured
    _check_github_issue
    ;;
  ...
  archive)
    if [[ ! -f "$CHANGE_DIR/verify-report.md" ]]; then
      fail "Missing verify-report.md — run specpaper verify first"
    fi
    ;;
